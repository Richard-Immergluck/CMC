...
test('anonymous visitor can open a catalogue track and return to the listing', async ({ page }) => {
  ...
  await expect(page.getByText('Log in or create an account to add this track to your cart.')).toBeVisible()
  await expect(page.getByRole('link', { name: 'Log In / Sign Up' })).toHaveAttribute('href', /\/auth\/signin\?callbackUrl=%2Fcatalogue%2F\d+/)
...
test('anonymous visitor can search catalogue tracks', async ({ page }) => {
  ...
  const resultCards = page.locator('.cmc-catalogue-track-row')
  ...
  await expect(resultCards.locator('[data-label="Composer"]').filter({ hasText: /Bach/i })).toHaveCount(0)
  await expect(page.getByLabel('Composer').locator('option')).toHaveText([
    'All',
    'Mendelssohn Style Synthetic Fixture'
  ])
  const keyOptions = await page.getByLabel('Key').locator('option').allTextContents()
  expect(keyOptions[0]).toBe('All')
  expect(keyOptions).toContain('C major')
