...
test('app router shell and first public routes are present', () => {
  ...
  const appUploadBatchTrackRoute = fs.readFileSync(path.join(root, 'app/api/upload-batches/[batchId]/tracks/[trackId]/route.js'), 'utf8')
  const appBulkTrackMetadataRoute = fs.readFileSync(path.join(root, 'app/api/tracks/bulk-metadata/route.js'), 'utf8')
  ...
  assert.match(appUploadBatchTrackRoute, /requireTrustedRouteOrigin/)
  assert.match(appUploadBatchTrackRoute, /removeFailedTrackFromUploadBatch/)
  assert.match(appBulkTrackMetadataRoute, /requireTrustedRouteOrigin/)
  assert.match(appBulkTrackMetadataRoute, /requireTrackUploadPermission/)
  assert.match(appBulkTrackMetadataRoute, /bulkUpdateUploadedTrackMetadata/)
  assert.match(appAdminBulkTrackModerationRoute, /requireTrustedRouteOrigin/)
  assert.match(appAdminBulkTrackModerationRoute, /requireSensitiveRouteCurrentUser/)
  assert.match(appAdminBulkTrackModerationRoute, /requireSupportPermission/)
