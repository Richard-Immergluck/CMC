...
export const hasBlockedReleaseDependency = release => {
  if (!release || release.status !== 'NEEDS_CHANGES') {
    return false
  }
...
export const releaseMatchesAdminFilter = ({ filter, release }) => {
  ...
  switch (filter) {
    ...
    case adminWorksCollectionFilters.blockedDependencies:
      return hasBlockedReleaseDependency(release)
...
export const getAdminWorksCollectionFilterCounts = releases => {
  const initialCounts = {
    ...
    [adminWorksCollectionFilters.blockedDependencies]: 0,
