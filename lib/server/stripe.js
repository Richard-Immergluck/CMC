    ...
    checkout: {
      sessions: {
        create: async checkoutOptions => {
          ...
          const mode = checkoutOptions.metadata?.e2eCheckoutMode || 'paid'
          const id = `cs_e2e_${mode}_${orderId}`

          if (mode === 'cancel') {
            return {
              id,
              url: checkoutOptions.cancel_url
            }
          }
        ...
        retrieve: async sessionId => ({
          id: sessionId,
          payment_status: sessionId.startsWith('cs_e2e_unpaid_') ? 'unpaid' : 'paid',
