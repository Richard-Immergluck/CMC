...
import { parseAuditCleanupRetentionDays } from './audit-cleanup-core.mjs'
...
export const getSecurityDashboardData = async () => {
  ...
  const auditActionCounts = toAuditActionCounts(groupedAuditEvents)
  ...
  return buildSecurityDashboardSummary({
    ...
    accountLifecycleSummary: buildAccountLifecycleSummary({
      accountStatusCounts,
      auditActionCounts
    }),
    auditRetentionStatus: buildAuditRetentionStatus({
      cleanupCandidateCount,
      oldestAuditEvent,
      retentionDays
    }),
