...
export const buildAccountLifecycleSummary = ({
  accountStatusCounts = [],
  auditActionCounts = {}
}) => {
  const accounts = {
  ...
  for (const statusCount of accountStatusCounts) {
    const status = String(statusCount.accountStatus || '').toLowerCase()

    if (Object.hasOwn(accounts, status)) {
      accounts[status] = statusCount._count?._all || statusCount.count || 0
    }
  }

  const lifecycleEvents = {
    inactiveApiRejected: auditActionCounts['auth.inactive_api_rejected'] || 0,
    signInDenied: auditActionCounts['auth.sign_in_denied'] || 0,
    signOut: auditActionCounts['auth.sign_out'] || 0,
    accessUpdated: auditActionCounts['user_access.updated'] || 0
  }
  const inactiveAccounts = accounts.suspended + accounts.closed
  const rejectionEvents = lifecycleEvents.inactiveApiRejected + lifecycleEvents.signInDenied

  return {
    status: inactiveAccounts > 0 || rejectionEvents > 0 ? 'review' : 'clear',
    accounts,
    lifecycleEvents,
    inactiveAccounts,
    rejectionEvents
