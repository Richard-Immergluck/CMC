...
export const uploadBatchTrackIdParamSchema = yup
  .object({
    batchId: positiveInteger.required(),
    trackId: positiveInteger.required()
  })
...
export const trackRequestResponseBodySchema = yup
  .object({
    ...
    pricePence: optionalPositiveInteger,
    pricingJustification: yup.string().trim().max(2000).optional(),
    rejectionNote: yup.string().trim().max(1000).optional(),
    rejectionReason: yup.string().trim().max(120).optional(),
    responseNote: yup.string().trim().max(2000).optional(),
    saleFormat: yup.string().oneOf([saleFormats.individual]).default(saleFormats.individual),
    status: yup.string().oneOf(['ACCEPTED', 'DECLINED']).required()
  })
  .test('accepted-response-requires-price', 'Accepted request responses require a platform-approved buyer price', value => {
    if (!value) {
      return false
    }
...
export const updateTrackMetadataBodySchema = yup
  .object({
    additionalInfo: yup.string().trim().min(1).max(2000).optional(),
    composer: yup.string().trim().min(1).max(255).optional(),
    downloadName: yup.string().trim().max(255).optional(),
    instrumentation: yup.string().trim().min(1).max(255).optional(),
    key: yup.string().trim().min(1).max(255).optional(),
    title: yup.string().trim().min(1).max(255).optional()
  })
  .test('has-track-metadata-update', 'At least one track metadata field is required', value => {
    return Boolean(
...
export const bulkUpdateTrackMetadataBodySchema = yup
  .object({
    trackIds: yup
      .array()
      ...
      .max(50)
      .required(),
    metadata: updateTrackMetadataBodySchema.required()
  })
  .test('bulk-track-ids-unique', 'Each track can only be selected once', value => {
...
const worksCollectionBodyShape = {
  ...
  tagSlugs: yup
    .array()
    .of(yup.string().oneOf(worksCollectionTagSlugs).required())
    .min(1, 'Choose at least one discovery tag')
    .max(maxWorksCollectionTags, `Choose ${maxWorksCollectionTags} discovery tags or fewer`)
    .test('works-collection-tags-unique', 'Choose each discovery tag once', value => {
...
export const adminBulkTrackModerationBodySchema = yup
  .object({
    decision: yup.string().oneOf(['approve', 'reject']).required(),
    moderationNotes: yup.string().trim().max(2000).optional(),
    trackIds: yup.array().of(positiveInteger.required()).min(1).max(50).required()
  })
  .test('bulk-admin-track-ids-unique', 'Each track can only be selected once', value => {
...
export const adminPricingReviewBodySchema = yup
  .object({
    ...
    targetType: yup.string().oneOf(['track', 'requestProposal', 'requestResponse', 'release']).required()
  })
  .noUnknown(true)
