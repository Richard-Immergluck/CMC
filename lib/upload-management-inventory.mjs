...
const publicReadyTrackStatus = {
  moderationStatus: 'APPROVED',
  processingStatus: 'READY',
  status: 'PUBLISHED'
}

export const hasBlockedCollectionDependency = collection => {
  if (!collection || collection.status !== 'NEEDS_CHANGES') {
    return false
  }
  ...
  return (collection.tracks || []).some(track => {
    ...
    return Boolean(hasLifecycleContext) && (
      track.moderationStatus !== publicReadyTrackStatus.moderationStatus ||
      track.processingStatus !== publicReadyTrackStatus.processingStatus ||
      track.status !== publicReadyTrackStatus.status
    )
  })
}
...
export const collectionMatchesUploadInventoryFilter = ({ collection, filter }) => {
  ...
  switch (filter) {
    ...
    case uploadInventoryCollectionFilters.blockedDependencies:
      return hasBlockedCollectionDependency(collection)
...
export const getUploadInventoryCollectionFilterCounts = collections => {
  const initialCounts = {
    ...
    [uploadInventoryCollectionFilters.blockedDependencies]: 0,
